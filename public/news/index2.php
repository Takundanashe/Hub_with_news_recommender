...
// Log an impression for every post shown in this feed load — this is what
// lets the recommender later distinguish "shown but ignored" from posts
// the user never saw at all.
if ($posts) {
    $impStmt = $db->prepare(
        'INSERT INTO news_impressions (news_id, user_id) VALUES (:nid, :uid)'
    );
    $db->beginTransaction();
    foreach ($posts as $p) {
        $impStmt->execute([':nid' => (int) $p['internal_id'], ':uid' => $userId]);
    }
    $db->commit();
}
